...
// Funções auxiliares internas (sem alterações)
    ...
    if (item.dados && item.dados.length > 0) {
      ...
      const linhasParaAdicionar = item.dados.map(objetoDado => {
        return cabecalhos.map(cabecalho => {
          const chaveOuValor = mapaChaves[item.aba][cabecalho];
          if (chaveOuValor === 'Pendente' || chaveOuValor === '') {
            return chaveOuValor;
          }
          const valor = objetoDado[chaveOuValor];
          return valor !== undefined && valor !== null ? valor : '';
        });
      });
      aba.getRange(linhaInicial, 1, numLinhasAdicionadas, cabecalhos.length).setValues(linhasParaAdicionar);
...
// [CORRIGIDO] Função modificada para incluir os dados das faturas
function ConciliacaoNFCrud_obterDadosGeraisDasNFs(chavesAcessoNF) {
    ...
    const cabecalhosTrib = dadosTrib.shift();
    const colMapTrib = {};
    cabecalhosTrib.forEach((header, index) => { colMapTrib[header] = index; });

    const resultadosMap = {};

    for (const linha of dadosTrib) {
        const chaveAtual = linha[colMapTrib["Chave de Acesso"]];
        if (chavesSet.has(chaveAtual)) {
            resultadosMap[chaveAtual] = {
              chaveAcesso: chaveAtual,
              totalBaseCalculoIcms: parseFloat(linha[colMapTrib["Total Base Cálculo ICMS"]]) || 0,
              totalValorIcms: parseFloat(linha[colMapTrib["Total Valor ICMS"]]) || 0, totalValorIcmsSt: parseFloat(linha[colMapTrib["Total Valor ICMS ST"]]) || 0,
              totalValorProdutos: parseFloat(linha[colMapTrib["Total Valor Produtos"]]) || 0, totalValorFrete: parseFloat(linha[colMapTrib["Total Valor Frete"]]) || 0,
              totalValorSeguro: parseFloat(linha[colMapTrib["Total Valor Seguro"]]) || 0, totalValorDesconto: parseFloat(linha[colMapTrib["Total Valor Desconto"]]) || 0,
              totalValorIpi: parseFloat(linha[colMapTrib["Total Valor IPI"]]) || 0, totalValorPis: parseFloat(linha[colMapTrib["Total Valor PIS"]]) || 0,
              totalValorCofins: parseFloat(linha[colMapTrib["Total Valor COFINS"]]) || 0, totalOutrasDespesas: parseFloat(linha[colMapTrib["Total Outras Despesas"]]) || 0,
              valorTotalNf: parseFloat(linha[colMapTrib["Valor Total da NF"]]) || 0,
              faturas: [] // Inicializa um array vazio para as faturas
            };
        }
    }

    // 2. Busca as faturas e as agrupa por chave de acesso
    const abaFat = planilha.getSheetByName(ABA_NF_FATURAS);
    const dadosFat = abaFat.getDataRange().getValues();
    const cabecalhosFat = dadosFat.shift();
    const colMapFat = {
        chave: cabecalhosFat.indexOf("Chave de Acesso"),
        numFat: cabecalhosFat.indexOf("Número da Fatura"),
        numParc: cabecalhosFat.indexOf("Número da Parcela"),
        venc: cabecalhosFat.indexOf("Data de Vencimento"),
        valor: cabecalhosFat.indexOf("Valor da Parcela")
    };

    for (const linha of dadosFat) {
      const chaveAtual = linha[colMapFat.chave];
      if (resultadosMap[chaveAtual]) { // Se a chave da fatura corresponde a uma NF que estamos processando
        const dataVenc = linha[colMapFat.venc];
        resultadosMap[chaveAtual].faturas.push({
          numeroFatura: linha[colMapFat.numFat],
          numeroParcela: linha[colMapFat.numParc],
          dataVencimento: dataVenc instanceof Date ? dataVenc.toISOString() : dataVenc,
          valorParcela: parseFloat(linha[colMapFat.valor]) || 0
        });
      }
    }
...
function ConciliacaoNFCrud_atualizarMapeamentoConciliacao(novosMapeamentos) {
  ...
  try {
    const planilha = SpreadsheetApp.getActiveSpreadsheet();
    let aba = planilha.getSheetByName(ABA_CONCILIACAO);
    if (!aba) {
      aba = planilha.insertSheet(ABA_CONCILIACAO);
      aba.appendRow(["Item da Cotação", "Descrição Produto (NF)", "GTIN/EAN (Cód. Barras)"]);
      Logger.log(`Aba "${ABA_CONCILIACAO}" não encontrada. Foi criada uma nova.`);
    }
    const dadosAtuaisRange = aba.getDataRange();
    const dadosAtuais = dadosAtuaisRange.getValues();
    const cabecalhos = dadosAtuais.shift() || ["Item da Cotação", "Descrição Produto (NF)", "GTIN/EAN (Cód. Barras)"];
    const colItemCotacao = cabecalhos.indexOf("Item da Cotação");
    const colDescricaoNF = cabecalhos.indexOf("Descrição Produto (NF)");
    const colGtin = cabecalhos.indexOf("GTIN/EAN (Cód. Barras)");
    if (colItemCotacao === -1 || colDescricaoNF === -1 || colGtin === -1) {
      Logger.log(`ERRO: Uma ou mais colunas ("Item da Cotação", "Descrição Produto (NF)", "GTIN/EAN (Cód. Barras)") não foram encontradas na aba "${ABA_CONCILIACAO}".`);
      return;
    }
